  ...
  return (
    <>
              ...
              return (
                <div key={item.name} className="relative group">
                    ...
                    className={`
                      ...
                      ${isActive
                        ...
                        : item.locked
                          ...
                          : 'bg-transparent text-gray-400 hover:text-white border border-transparent hover:border-white/20'
                      }
                    `}
                  ...
                  {hoveredIcon === item.name && (
                    <div className="absolute left-16 top-1/2 -translate-y-1/2 bg-black text-white text-xs px-3 py-1.5 rounded-lg whitespace-nowrap pointer-events-none animate-fadeIn border border-white/20 shadow-xl shadow-black">
          ...
          {user && (
            <div className="relative group">
              <button
                ...
                className="w-11 h-11 rounded-xl bg-transparent text-gray-400 hover:text-white transition-all duration-200 flex items-center justify-center border border-transparent hover:border-white/20 hover:scale-105"
                title="Logout"
      ...
      <div className="hidden md:block lg:hidden fixed left-4 top-1/2 -translate-y-1/2 z-50">
        <div className="bg-black border border-gray-800/60 backdrop-blur-md rounded-xl p-3 flex flex-col items-center gap-4 shadow-2xl shadow-black/50">
          {navItems.map((item) => {
            const IconComponent = item.icon;
            const isActive = currentPage === item.name;
            ...
            return (
              <button
                ...
                className={`
                  ...
                  ${isActive
                    ? 'bg-white/10 text-white shadow-md shadow-black/50'
                    : item.locked
                      ? 'text-gray-600'
          ...
          {user && (
            <>
              ...
              <button
                onClick={handleLogoutClick}
                className="w-10 h-10 rounded-lg text-gray-400 hover:text-white transition-all duration-200 flex items-center justify-center active:scale-95"
              >
                <FiLogOut size={18} />
              </button>
            </>
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-50">
        <div className="bg-black border-t border-gray-800/80 shadow-2xl shadow-black/50">
          <div className="flex items-stretch justify-around max-w-md mx-auto">
              ...
              return (
                <button
                  ...
                  className={`
                    ...
                    ${item.locked ? 'cursor-not-allowed' : 'active:bg-white/5'}
                    ${isActive ? 'text-white' : 'text-gray-500'}
                  `}
                >
                  {/* Active background glow */}
                  {isActive && (
                    <div className="absolute inset-x-2 top-1 bottom-1 bg-white/5 rounded-xl border border-white/10"></div>
                  )}
                  <div className="relative z-10">
                    <IconComponent size={22} strokeWidth={isActive ? 2.5 : 2} />
                    {item.locked && (
                      <div className="absolute -top-1 -right-2 bg-black rounded-full p-0.5">
                        <FiLock size={10} className="text-gray-500" />
                      </div>
                    )}
                  </div>
                  <span className={`text-[10px] font-medium relative z-10 ${isActive ? 'text-white' : 'text-gray-500'}`}>
                    {item.label}
                  </span>
                  {/* Active indicator line */}
                  {isActive && <div className="w-4 h-0.5 rounded-full bg-white mt-0.5 relative z-10"></div>}
                </button>
              );
            })}
            ...
            {user && (
              <button
                onClick={handleLogoutClick}
                className="flex flex-col items-center justify-center gap-1 py-3 px-4 flex-1 text-gray-500 transition-all duration-200 min-h-[64px] active:bg-white/5"
              >
      ...
      {showLogoutConfirm && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[60] backdrop-blur-sm p-4">
          <div className="bg-black border border-gray-800 rounded-2xl p-6 sm:p-8 w-full max-w-sm mx-auto shadow-2xl shadow-black/50">
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className="flex-1 bg-white/10 hover:bg-white/20 text-white px-4 py-2.5 rounded-lg font-medium transition border border-white/20 text-sm sm:text-base"
              >
                Logout
              </button>
            </div>
